      ...
      const saveFile = async (certId: number) => {
        if (req.file) {
          ...
          await pool.execute(
            ...
            [certId, req.file.originalname, req.file.originalname, req.file.mimetype, file_type || 'certificate', req.file.buffer]
          );
        }
      };
  ...
  app.post('/api/certificates/:id/files', authenticate, upload.single('file'), async (req: any, res) => {
    ...
    try {
      const [insertResult]: any = await pool.execute(
        'INSERT INTO files (certificate_id, filename, original_name, mimetype, file_type, data) VALUES (?, ?, ?, ?, ?, ?)', 
        [req.params.id, req.file.originalname, req.file.originalname, req.file.mimetype, file_type || 'certificate', req.file.buffer]
      );
