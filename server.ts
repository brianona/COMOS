  ...
  try {
    ...
    try {
      const tables = [
        ...
        'fuel_analysis_reports', 'lube_oil_ldr_reports', 'lube_oil_analysis_reports'
      ];
...
`;
  ...
  app.get('/api/lube-oil-analysis-reports', authenticate, async (req: any, res) => {
    try {
      ...
      for (const report of reports) {
        const [files]: any = await pool.execute(
          'SELECT id, filename, size FROM lube_oil_analysis_files WHERE report_id = ?',
          [report.id]
        );
        reportsWithFiles.push({
          id: String(report.id),
  ...
  app.get('/api/lube-oil-analysis-files/:id', authenticate, async (req, res) => {
    try {
      const [files]: any = await pool.execute(
        'SELECT * FROM lube_oil_analysis_files WHERE id = ?',
        [req.params.id]
      );
      if (files.length > 0 && files[0].data) {
        const file = files[0];
    ...
    try {
      const [result]: any = await pool.execute(
        `INSERT INTO lube_oil_analysis_reports (
          ...
          viscosity, water_content, tbn, insolubles, status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          vesselId ?? null,
          date ?? null,
  ...
  app.put('/api/lube-oil-analysis-reports/:id', authenticate, async (req: any, res) => {
    ...
    try {
      ...
      if (keptFileIds.length > 0) {
        const placeholders = keptFileIds.map(() => '?').join(',');
        await pool.execute(
          `DELETE FROM lube_oil_analysis_files WHERE report_id = ? AND id NOT IN (${placeholders})`,
          [id, ...keptFileIds]
        );
      } else {
        await pool.execute(
          'DELETE FROM lube_oil_analysis_files WHERE report_id = ?',
          [id]
        );
      }
  ...
  app.get('/api/requisition-attachments/:id', authenticate, async (req: any, res) => {
    ...
    try {
      const [rows]: any = await pool.execute(
        'SELECT filename, mimetype, data FROM requisition_attachments WHERE id = ?',
        [id]
      );
      if (rows.length === 0) {
